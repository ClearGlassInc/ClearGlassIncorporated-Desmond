...
The first commercial profile of ClearGlassInc Artemis is **ClearGlass AEGIS Federal**, a
continuous cyber-regulatory assurance control plane for Canadian federally regulated and
critical-infrastructure organizations. It turns evidence already present in Microsoft 365,
Azure, AWS, GitHub, endpoint, SIEM, policy, incident, continuity, and vendor systems into a
reviewable evidence graph. This profile is compliance engineering—not legal advice—and every
control interpretation, statutory effective date, designation decision, filing deadline, and
regulator submission must be validated by qualified Canadian counsel and the accountable
compliance owner against authoritative sources before use.

### Commercial outcome and product boundary

The product sells one attributable outcome: **existing security evidence becomes an auditable,
continuously monitored regulatory compliance system**. It does not replace counsel, an auditor,
a SOC, incident response, or an organization's accountable executives. It also never infers that
an organization is compliant merely because a connector is healthy or an artifact exists.
Evidence supports a control only after deterministic applicability, freshness, provenance,
quality, and owner-review rules pass.

| Product surface | Delivery contract | Governed output |
|---|---|---|
| C-8 Readiness Strike | Time-boxed discovery of assets, critical systems, vendors, IAM, vulnerabilities, monitoring, incidents, continuity evidence, and control gaps. | Draft applicability record, evidence-backed risk register, remediation plan, and executive package for human validation. |
| Continuous Assurance | Scheduled and event-driven collection plus control verification. | Live `Requirement → Control → Evidence → Owner → Risk → Remediation → Verification` view and versioned evidence bundles. |
| Vendor Assurance Network | Tenant-isolated vendor intake, evidence requests, attestations, monitoring, deterioration alerts, and reassessment. | Supplier assurance record with explicit evidence scope, expiry, confidence, exceptions, and reviewer decision. |
| AI Assurance Extension | Inventory and assessment of generative and agentic AI systems, data flows, tools, models, vendors, evaluations, and approvals. | AI-system risk case linked to cyber, technology, operational-resilience, privacy, and third-party controls. |

The launch ontology may encode mappings for CCSPA / Bill C-8, applicable Cyber Centre guidance,
OSFI B-13, B-10, and E-21, supply-chain controls, and AI-risk controls. Those mappings are
**versioned interpretations**, not hard-coded legal truth. A regulatory-content maintainer drafts
each mapping; counsel or an authorized compliance owner approves it; a second reviewer verifies
the source citation and effective window; Apollo promotes the signed mapping bundle. Superseded
mappings remain queryable so an evidence package can be reproduced as of its original assessment
date.

### Regulatory evidence graph

```mermaid
flowchart LR
  authority[Authority / Guidance] --> requirement[Requirement]
  requirement --> mapping[Applicability Mapping]
  mapping --> control[Control]
  control --> system[System / Critical Cyber System]
  system --> evidence[Evidence Artifact]
  control --> owner[Accountable Owner]
  control --> risk[Risk / Exception]
  risk --> remediation[Remediation]
  remediation --> verification[Independent Verification]
  vendor[Third Party] --> service[Product / Service]
  service --> system
  vendor --> evidence
  incident[Cyber Incident] --> system
  incident --> report[Reporting Decision]
  report --> approval[Human Approval]
```

Every node carries tenant, jurisdiction, mission/purpose, classification, compartments,
valid-time and system-time, source URI, content digest, collector version, evidence freshness,
confidence, and access policy. Every edge is itself attributable and temporal. A missing edge is
reported as an evidence gap; it is never filled by a model. Model-generated classifications and
mappings are proposals with citations and confidence until accepted by a named reviewer.

Minimum profile objects extend the core ontology with `Authority`, `Requirement`,
`ApplicabilityDecision`, `Control`, `ControlTest`, `EvidenceArtifact`, `CriticalCyberSystem`,
`ThirdParty`, `ProvidedService`, `Attestation`, `Finding`, `RiskAcceptance`, `Remediation`,
`Verification`, `CyberIncident`, `ReportingDecision`, and `EvidencePackage`. Important links
include `IMPLEMENTS`, `SUPPORTED_BY`, `APPLIES_TO`, `DEPENDS_ON_VENDOR`, `SUPERSEDES`,
`OWNED_BY`, `REMEDIATES`, `VERIFIED_BY`, and `REPORTED_UNDER`.

### Connector-to-evidence pipeline

1. A tenant-scoped, read-only connector acquires the smallest required data set with a short-lived
   workload identity. Raw secrets, message bodies, source code, and personal data are excluded by
   default.
2. Foundry lands an immutable bronze record, verifies schema and source identity, computes a
   digest, applies classification and retention, and quarantines malformed or unexpectedly broad
   payloads.
3. A deterministic silver transform normalizes resources and relationships. A separate gold
   transform produces candidate evidence observations; an AIP agent may annotate but cannot
   establish compliance.
4. The policy service checks tenant, purpose, need-to-know, coalition/releasability, collection
   scope, and data residency before an Ontology object or link is created.
5. A control test evaluates explicit predicates, freshness, completeness, and compensating-control
   rules. Failed or unknown tests open a draft finding; they do not automatically escalate severity
   or notify an external party.
6. Foundry Ontology Actions prepare evidence requests, remediation tasks, risk acceptances, and
   reporting packages. Consequential actions remain pending until the authorized human approves
   the immutable payload.
7. Gotham presents investigations, affected-asset graphs, vendor dependencies, incident timelines,
   and evidence gaps. AIP drafts summaries and answers questions only from policy-filtered objects
   with object-level citations.
8. Apollo deploys signed connector, mapping, prompt, workflow, and policy bundles through shadow
   and canary rings with a last-known-good rollback pointer.

### Regulatory decision state machines

A compliance conclusion uses `UNASSESSED → EVIDENCE_PENDING → TESTED → REVIEW_PENDING →
EFFECTIVE | INEFFECTIVE | NOT_APPLICABLE | EXCEPTION_APPROVED`. Only deterministic tests may move
a record to `TESTED`; only an authorized reviewer may establish the terminal assessment. Evidence
expiry returns the assessment to `EVIDENCE_PENDING` without deleting its history.

A potential incident uses `OBSERVED → TRIAGED → REPORTABILITY_REVIEW → PENDING_HUMAN_APPROVAL →
SUBMITTED | NOT_REPORTABLE | CLOSED`. Detection may open and enrich a case automatically. The
platform may draft a notification, calculate a candidate deadline from an approved rule bundle,
and assemble evidence; it cannot decide legal reportability or transmit to a regulator without the
required accountable approval. A submission with an ambiguous network outcome enters
`RECONCILIATION_REQUIRED` and is never blindly retried.

A vendor finding uses `DETECTED → EVIDENCE_REQUESTED → VENDOR_RESPONDED → OWNER_REVIEW →
REMEDIATE | ACCEPT | DISPUTE | CLOSE`. Scores are decision support, not facts: the UI must expose
source age, missing evidence, weighting version, uncertainty, and any human override. A vendor
cannot view another vendor's data, an enterprise cannot reuse evidence beyond its consent and
contractual purpose, and aggregate benchmarks require privacy review and minimum cohort controls.

### Acceptance criteria for the first regulated pilot

- One synthetic tenant completes Microsoft Entra/Azure, AWS, GitHub, EDR/SIEM, policy, incident,
  continuity, and vendor-evidence ingestion with least-privilege read access, replay, deduplication,
  quarantine, and connector revocation demonstrated.
- A versioned requirement bundle maps at least one approved requirement through a control and
  critical system to fresh evidence, owner, finding, remediation, verification, and reproducible
  executive package; every assertion resolves to source lineage.
- Cross-tenant, cross-compartment, expired-purpose, unauthorized property, poisoned document, and
  prompt-injection tests fail closed before retrieval or tool use.
- No AI output can mark a control effective, accept a risk, decide incident reportability, contact a
  vendor or regulator, change collection scope, or promote a mapping without deterministic policy
  and the configured human approval.
- Evidence deletion follows retention and legal-hold policy while the audit plane retains the
  permitted tamper-evident decision metadata; backups, restore, connector disablement, evidence
  package reproduction, and Apollo rollback are exercised.
- Pilot metrics report evidence coverage, verified freshness, false-positive and false-negative
  control tests, time-to-owner, time-to-remediate, package reproduction success, policy denials,
  connector error budget, p95 latency, operator override rate, and reviewer trust—without claiming
  unmeasured compliance or business outcomes.
