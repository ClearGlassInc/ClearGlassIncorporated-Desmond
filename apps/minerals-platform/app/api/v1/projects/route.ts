...
export async function GET(request: NextRequest) {
  try {
    ...
    const [items, total] = await Promise.all([
      db.project.findMany({ where, include: { mineral: true, country: true, jurisdiction: true, operator: true, ownership: { include: { ownerCompany: true } }, risks: { orderBy: { observedAt: "desc" }, take: 1 } }, skip: (input.page - 1) * input.pageSize, take: input.pageSize, orderBy: { name: input.order } }),
