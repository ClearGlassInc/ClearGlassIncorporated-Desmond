...
const createSchema = z.object({
  ...
  sourceSnapshot: z.record(z.string(), z.json()).optional(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = JSON.parse(JSON.stringify(input.body)) as Prisma.InputJsonObject;
    const sourceSnapshot = input.sourceSnapshot ? JSON.parse(JSON.stringify(input.sourceSnapshot)) as Prisma.InputJsonObject : undefined;
    const report = await db.report.create({ data: { organizationId: principal.organizationId, authorId: principal.userId, title: input.title, status: "DRAFT", body, sourceSnapshot, generatedByAi: input.generatedByAi } });
