 ...
 * Dependency-free, progressive enhancement. No content is hidden until this
 * runtime has successfully initialized an observer, and every animated state
 * degrades to static HTML/SVG under reduced-motion or constrained devices.
 */
(function () {
  "use strict";
  ...
  function configureHeroMotion(hero) {
    ...
    function setLive() {
      hero.classList.toggle("cg-motion-live", visible && pageVisible && !reduced);
    }
  ...
  function configureSectionTransitions() {
    ...
    sections.forEach(function (section, index) {
      /* Do not hide near-viewport content at initialization. */
      if (index > 0 && section.getBoundingClientRect().top > innerHeight * .82) {
    ...
    /* Fail open: content must never remain invisible if an observer stalls. */
    setTimeout(function () {
