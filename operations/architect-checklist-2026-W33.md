...
| 7 | Trial one vuln-detection AI tool in non-prod pipeline | ✅ Audited — already deployed | `.github/workflows/codeql.yml` (existing) |

---

## 1 — CI/CD permission + secret-exposure audit ✅

**Method:** enumerated all 79 workflow files in `.github/workflows/`, checked for a
...
- 79 workflow files is a large surface with visible overlap between the several
...
- **Cost drivers to model:** (a) model/API inference spend — the dominant AI
  ...
  GPU node pool (fixed monthly per node — see radar); (c) CI minutes — ~79
  workflow files incl. several scheduled loops (`commerce-daily-loop`,
...
## 7 — Trial a vuln-detection AI tool in a non-prod pipeline ✅ (audit)

**Finding:** this item is already satisfied. The repo runs **GitHub CodeQL**
(semantic SAST) via **`.github/workflows/codeql.yml`** — it scans **Python** and
**JavaScript/TypeScript** (`build-mode: none`) on PR, push to `main`, and weekly,
analysis-only (uploads to the Code Scanning tab, not a merge gate). It is
least-privilege and SHA-pinned, consistent with the rest of the estate.

I initially added a second `codeql-trial.yml`; on discovering the existing
workflow (my branch base predated it) I **removed it** — a duplicate would collide
on the `/language:python` and `/language:javascript-typescript` SARIF categories
and waste CI. Lesson logged: rebase onto `main` before auditing the workflow set.

**Recommendations (not forced this week):**

- **Tune the existing CodeQL**, don't add a redundant one: raise recall with the
  `security-extended` query pack in `codeql.yml`, and — more importantly — confirm
  the Code Scanning alerts are actually **triaged**, not just generated. A SAST
  lane that no one reads is theatre.
- **For a genuinely new tool trial**, add a *complementary* scanner (e.g. Semgrep)
  only where it catches classes CodeQL misses — tracked in `TECH_RADAR.md` under
  Assess. Do not duplicate CodeQL's language coverage.
...
- `operations/architect-checklist-2026-W33.md` — this report (items 1–7 findings
  + plans). Item 7 is an audit: CodeQL SAST already exists in the repo, so the
  duplicate workflow I first added was removed rather than shipped.
