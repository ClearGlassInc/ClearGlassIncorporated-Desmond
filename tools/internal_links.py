...
# outside the generated public journey graph. Keeping this inventory beside
# PAGES makes the site-wide audit exhaustive without leaking private consoles
# into search or adding conversion UI to redirects, loaders, and completion pages.
...
# Purpose-built pages may carry their own semantic breadcrumb/navigation and
# therefore must not also receive the generated journey module. This is an
# explicit, reviewable allowlist rather than a directory-level exemption.
NATIVE_JOURNEY_PAGES: frozenset[str] = frozenset()

# --------------------------------------------------------------------------
# Site graph: every generated-journey page, with descriptive anchor text.
# --------------------------------------------------------------------------
PAGES: dict[str, tuple[str, str]] = {
    ...
    "blog/aethersense-wifi-csi-human-sensing.html": ("AetherSense Wi-Fi Sensing", "local-first human sensing without cameras"),
    "blog/biwei-zhang-nato-shape-open-source-intelligence-dossier.html": ("NATO SHAPE OSINT Case Study", "a public-record open-source intelligence dossier"),
    "blog/2027-outlook-transition-risk-monitoring.html": ("2027 Outlook", "transition, valuation and risk monitoring for the year ahead"),
...
CLUSTERS: dict[str, dict] = {
    ...
    "healthcare": {"name": "Healthcare Intelligence", "pillar": "clearpulse.html", "members": ["clearpulse-architecture.html", "offers/phipa-readiness.html", "offers/phipa-readiness-checklist.html"], "cta": [("offers/phipa-readiness.html", "Get the free PHIPA readiness checklist"), CTA_STORE]},
    "government": {"name": "Government & Procurement", "pillar": "government.html", "members": ["operations/procurement-readiness.html", "operations/federal-supplier-handoff.html", "counter-uas-commercialization-os.html", "traffic-enforcement.html", "sats-digital-twin.html", "minerals-platform.html"], "cta": [CTA_STORE, ("operations/procurement-readiness.html", "Check our procurement readiness")]},
    "services": {"name": "Services & Engagements", "pillar": "offers/index.html", "members": ["revenue.html", "store.html", "products/advanced-secure-systems-engineering.html", "pricing.html", "plans.html", "workspace.html", "workspace-email.html", "workspace-migration.html", "workspace-security.html", "workspace-vs.html", "checkout/index.html", "seo-authority-hub.html", "smb-cyber-trust-kit.html", "smb.html", "offers/security-quick-audit.html", "offers/security-quick-audit-sample-report.html", "offers/security-quick-audit-methodology.html", "streaming-growth-command-center/index.html", "offers/autonomous-threat-modeling.html", "offers/hardening-sprint.html", "offers/guardian-command-nexus-blueprint.html", "revenue-engine.html", "side-store.html", "products.html", "business-productivity-suite.html", "global-growth-engine.html", "apps/command-center/index.html"], "cta": [CTA_STORE, CTA_PRICING]},
...
CSS = (
    "#cg-related{margin:54px auto 0;max-width:1120px;padding:0 18px 38px;font-family:'Inter',system-ui,-apple-system,sans-serif;position:relative}"
    "#cg-related .cgr-box{position:relative;overflow:hidden;background:linear-gradient(rgba(34,211,238,.055) 1px,transparent 1px),linear-gradient(90deg,rgba(124,150,255,.05) 1px,transparent 1px),radial-gradient(circle at 12% 0,rgba(34,211,238,.16),transparent 34%),radial-gradient(circle at 88% 100%,rgba(168,85,247,.16),transparent 36%),linear-gradient(165deg,rgba(5,12,28,.97),rgba(9,7,24,.97));background-size:32px 32px,32px 32px,auto,auto,auto;border:1px solid rgba(34,211,238,.34);border-radius:16px;padding:24px 26px;color:#d8e4ff;box-shadow:0 18px 54px rgba(0,0,0,.44),0 0 34px rgba(34,211,238,.12),inset 0 0 0 1px rgba(255,255,255,.035)}"
    "#cg-related .cgr-box:before{content:'';position:absolute;inset:0;pointer-events:none;background:linear-gradient(90deg,transparent,rgba(34,211,238,.18),transparent);transform:translateX(-120%) skewX(-18deg);animation:cgr-scan 8s ease-in-out infinite}"
    "#cg-related .cgr-box:after{content:'';position:absolute;left:24px;right:24px;top:0;height:1px;background:linear-gradient(90deg,transparent,#22d3ee,#a855f7,transparent);box-shadow:0 0 18px rgba(34,211,238,.75)}"
    "#cg-related .cgr-crumb{position:relative;font-size:10.5px;letter-spacing:.2em;text-transform:uppercase;color:#8fb9ff;margin:0 0 12px}"
    "#cg-related .cgr-crumb a{color:#67e8f9;text-decoration:none;text-shadow:0 0 12px rgba(34,211,238,.35)}#cg-related .cgr-crumb a:hover{color:#fff;text-decoration:none}"
    ...
    "#cg-related .cgr-route a{display:flex;min-height:46px;align-items:center;padding:8px 10px;border-radius:10px;background:rgba(7,14,32,.52);border:1px solid rgba(124,150,255,.15);color:#cfe0ff;text-decoration:none;font-size:12px;line-height:1.35}"
    "#cg-related .cgr-route a:hover,#cg-related .cgr-route a:focus-visible{border-color:#22d3ee;color:#fff;box-shadow:0 0 18px rgba(34,211,238,.14)}#cg-related .cgr-route b{display:block;color:#67e8f9;font-size:9px;letter-spacing:.16em;text-transform:uppercase;margin-bottom:2px}"
    "#cg-related ul{position:relative;list-style:none;margin:0;padding:0;display:grid;grid-template-columns:repeat(auto-fit,minmax(248px,1fr));gap:8px 18px}"
    "#cg-related li a{display:block;padding:9px 11px;border-radius:10px;font-size:13.5px;line-height:1.45;color:#cfe0ff;text-decoration:none;border:1px solid rgba(124,150,255,.08);background:rgba(7,14,32,.36);transition:transform .14s ease,border-color .14s ease,background .14s ease,box-shadow .14s ease}"
    "#cg-related li a:hover,#cg-related li a:focus-visible{transform:translateY(-1px);background:rgba(34,211,238,.1);border-color:rgba(34,211,238,.42);color:#fff;box-shadow:0 0 22px rgba(34,211,238,.14)}#cg-related li a b{color:#9bdcff;font-weight:700;text-shadow:0 0 12px rgba(34,211,238,.28)}"
    "#cg-related .cgr-cta{position:relative;margin:16px 0 0;padding-top:14px;border-top:1px solid rgba(34,211,238,.18);font-size:13.5px;color:#a9b8df}#cg-related .cgr-cta a{color:#67e8f9;font-weight:700;text-decoration:none;border-bottom:1px solid rgba(34,211,238,.36)}#cg-related .cgr-cta a:hover{color:#fff;border-bottom-color:#fff}"
    "@media(max-width:620px){#cg-related .cgr-box{padding:21px 17px}#cg-related .cgr-route{grid-template-columns:1fr}#cg-related ul{grid-template-columns:1fr}}@keyframes cgr-scan{0%,58%{transform:translateX(-120%) skewX(-18deg);opacity:0}70%{opacity:1}100%{transform:translateX(220%) skewX(-18deg);opacity:0}}@media(prefers-reduced-motion:reduce){#cg-related .cgr-box:before{animation:none}}"
)
DOCK_CSS = "#cg-related.cgr-dock{position:fixed;left:14px;bottom:14px;margin:0;padding:0;max-width:none;z-index:2147483000}#cg-related.cgr-dock .cgr-tab{display:inline-block;cursor:pointer;font-size:10.5px;letter-spacing:.22em;text-transform:uppercase;color:#dbe4ff;padding:8px 12px;border-radius:9px;border:1px solid rgba(124,150,255,.42);background:linear-gradient(180deg,rgba(18,20,42,.92),rgba(11,12,28,.92));box-shadow:0 6px 22px rgba(0,0,0,.4);backdrop-filter:blur(6px);user-select:none}#cg-related.cgr-dock .cgr-tab:hover{color:#fff;border-color:rgba(124,150,255,.85)}#cg-related.cgr-dock .cgr-box{display:none;position:absolute;bottom:calc(100% + 8px);left:0;width:min(400px,92vw);max-height:62vh;overflow-y:auto}#cg-related.cgr-dock:hover .cgr-box,#cg-related.cgr-dock:focus-within .cgr-box{display:block}#cg-related.cgr-dock ul{grid-template-columns:1fr}"
...
def anchor(from_page: str, to_page: str) -> str:
    ...
    return f'<li><a href="{html.escape(rel(from_page, to_page), quote=True)}"><b>{html.escape(title)}</b> — {html.escape(desc)}</a></li>'


def cluster_of(page: str) -> str:
    for cid, cluster in CLUSTERS.items():
        if page == cluster["pillar"] or page in cluster["members"]:
            return cid
...
def related_targets(page: str) -> tuple[list[str], list[tuple[str, str]], str]:
    ...
    ids = list(CLUSTERS)
    if page == "index.html":
        targets = [CLUSTERS[item]["pillar"] for item in ids if CLUSTERS[item]["pillar"] != page]
        return targets, cluster["cta"], ""
    if page == cluster["pillar"]:
        targets = list(cluster["members"][:PILLAR_MAX_MEMBERS])
        index = ids.index(cid)
        for offset in (1, 2):
            targets.append(CLUSTERS[ids[(index + offset) % len(ids)]]["pillar"])
    else:
        members = cluster["members"]
        index = members.index(page)
        count = len(members)
        window = min(SIBLING_WINDOW, count - 1)
        targets = [members[(index + offset) % count] for offset in range(1, window + 1)]
    seen, output = {page}, []
    for target in EXTRA_LINKS.get(page, []) + targets:
        if target not in seen:
            seen.add(target)
            output.append(target)
    return output, cluster["cta"], cluster["pillar"] if page != cluster["pillar"] else ""
...
def route_link(page: str, target: str, label: str) -> str:
    ...
    return f'<a href="{html.escape(rel(page, target), quote=True)}"><span><b>{html.escape(label)}</b>{html.escape(title)}</span></a>'
...
def build_block(page: str) -> str:
    ...
    if page == "index.html":
        ...
        crumb += f' › <a href="{html.escape(rel(page, pillar), quote=True)}">{html.escape(name)}</a>'
        heading = "Continue exploring"
    else:
        crumb += f" › {html.escape(name)}"
        heading = f"Inside {name}"
    items = "\n      ".join(anchor(page, target) for target in targets)
    previous, hub, following = journey_targets(page)
    route = "\n      ".join((route_link(page, previous, "Previous"), route_link(page, hub, "Topic hub"), route_link(page, following, "Next in journey")))
    cta_html = " · ".join(f'<a href="{html.escape(rel(page, path), quote=True)}">{html.escape(label)}</a>' for path, label in cta)
    docked = page in FIXED_VIEWPORT
    css = CSS + DOCK_CSS if docked else CSS
    cls = ' class="cgr-dock"' if docked else ""
    tab = '  <div class="cgr-tab" tabindex="0" role="button" aria-label="Show related ClearGlass pages">⌁ Explore ClearGlass</div>\n' if docked else ""
    return (
        f"{START}\n<aside id=\"cg-related\"{cls}>\n  <style>{css}</style>\n"
        f'  <nav class="cgr-box" aria-label="Related ClearGlass pages">\n    <p class="cgr-crumb">{crumb}</p>\n    <h2>{html.escape(heading)}</h2>\n'
    ...
    for cid, cluster in CLUSTERS.items():
        for page in [cluster["pillar"], *cluster["members"]]:
            if page in clustered and page != "index.html":
                errors.append(f"{page}: in more than one cluster")
            clustered.add(page)
            if page not in PAGES:
                errors.append(f"{cid}: {page} missing from PAGES")
    for page in PAGES:
        if page not in clustered:
            errors.append(f"{page}: in PAGES but no cluster")
        if not (ROOT / page).is_file():
            errors.append(f"{page}: file not found")
    for page in NATIVE_JOURNEY_PAGES:
        if not (ROOT / page).is_file():
            errors.append(f"{page}: native journey page not found")
    for src, targets in EXTRA_LINKS.items():
        for target in [src, *targets]:
            if target not in PAGES:
                errors.append(f"EXTRA_LINKS: unknown page {target}")
    overlap = set(PAGES) & set(EXCLUDED_PAGES)
    for page in sorted(overlap):
        errors.append(f"{page}: both mapped and excluded")
    if set(PAGES) & NATIVE_JOURNEY_PAGES:
        errors.append("native journey pages must not also be mapped")
    if set(EXCLUDED_PAGES) & NATIVE_JOURNEY_PAGES:
        errors.append("native journey pages must not also be excluded")
    discovered = {path.relative_to(ROOT).as_posix() for path in ROOT.rglob("*.html") if not NON_SITE_DIRS & set(path.parts)}
    classified = set(PAGES) | set(EXCLUDED_PAGES) | set(NATIVE_JOURNEY_PAGES)
    for page in sorted(discovered - classified):
        errors.append(f"{page}: HTML page is not classified")
    for page in sorted((set(EXCLUDED_PAGES) | set(NATIVE_JOURNEY_PAGES)) - discovered):
        errors.append(f"{page}: classified HTML page not found")
    return errors


def build_report() -> str:
    lines = ["# ClearGlass Site-Wide Wiring Plan", "", "> Generated by `python3 tools/internal_links.py`. Edit the site graph in that generator, not this report.", "", "## Experience architecture", "", "Every mapped page keeps its original content and receives one additive, crawlable journey module immediately before `</body>`. The module provides a Home/topic breadcrumb, a deterministic previous–hub–next route, contextual related pages, and two conversion paths. Fixed-viewport command surfaces use the same module as a keyboard-focusable corner dock so their canvas remains intact.", "", "The journey model is: **discover → orient in a topic hub → explore an adjacent capability → deepen through evidence → choose a governed engagement**. High-risk operational actions remain outside this marketing navigation and retain their existing human-approval boundaries.", "", "## Additive navigation system", "", "- **Breadcrumb:** Home and the current topic pillar provide location and semantic hierarchy.", "- **Journey rail:** Previous, topic hub, and next links create a predictable cinematic sequence.", "- **Related module:** Rotated sibling links distribute discovery across every page in a cluster; curated bridges connect adjacent clusters.", "- **CTA bridge:** Each cluster ends in two relevant, non-coercive next steps such as offers, pricing, readiness, or booking.", "- **Responsive behavior:** Three route cards collapse to one column below 620px; related cards use an adaptive grid; motion is disabled when reduced motion is requested.", "- **SEO and accessibility:** Links are static HTML, labels are descriptive, navigation landmarks are named, focus states are visible, and all routes remain unchanged.", f"- **Audit coverage:** {len(PAGES)} public pages are connected and {len(EXCLUDED_PAGES)} utility, completion, prototype, or private pages are explicitly excluded.", "", "## Page-by-page flow map", "", "| Page | Role | Previous | Topic hub | Next | Conversion bridge |", "|---|---|---|---|---|---|"]
    for cluster in CLUSTERS.values():
        ...
        for page in [cluster["pillar"], *cluster["members"]]:
            ...
            lines.append(f"| `{page}` — {title} | {description} | {PAGES[previous][0]} | {PAGES[hub][0]} | {PAGES[following][0]} | {ctas} |")
    lines.extend(("", "## Explicitly excluded surfaces", "", "These pages remain intact but are not eligible for generated journey updates. Existing markup is preserved; excluding them prevents future search leakage, conversion loops after form completion, and interference with loaders or recovery states.", "", "| Page | Role | Exclusion rationale |", "|---|---|---|"))
    for page, (role, reason) in EXCLUDED_PAGES.items():
        lines.append(f"| `{page}` | {role} | {reason} |")
    lines.extend(("", "## Implementation and verification strategy", "", "1. Maintain page metadata, cluster membership, curated bridges, and fixed-viewport exceptions in `tools/internal_links.py`.", "2. Run `python3 tools/internal_links.py` after any page is added, renamed, or repositioned. This regenerates both HTML modules and this report.", "3. Run `python3 tools/internal_links.py --check` in review to reject stale generated modules or a stale flow map.", "4. Validate local destinations, semantic landmarks, keyboard focus, mobile layout, reduced motion, and browser-console output before Pages deployment.", "5. Roll back by reverting the generator commit and regenerating; original page content is never rewritten outside the delimited `cg-related` block.", "", "## Growth rule", "", "New pages must be assigned one clear journey role, one topic cluster, previous/next adjacency, descriptive anchors, and a governed conversion bridge. This keeps the experience extensible without flattening product, editorial, operational, or legal content.", ""))
    return "\n".join(lines)
...
def main() -> int:
    ...
    for page in PAGES:
        if check:
            ...
            if not match or match.group(0) != build_block(page):
                print(f"stale: {page}")
                changed += 1
        else:
